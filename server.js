#!/usr/bin/env node
/**
 * openclaw-router — Local cost-optimizing proxy for Anthropic-compatible models.
 *
...
function proxyToAnthropic(req, res, body, routedModel, tier) {
  ...
  if (tier === "LIGHT") {
    ...
    if (req.headers["anthropic-beta"]) {
      ...
      if (betas.length > 0) req.headers["anthropic-beta"] = betas.join(",");
  ...
  if (isOpenRouter) {
    ...
    options.headers["HTTP-Referer"] = config.openRouterReferer || "https://github.com/whongsen934-ship-it/openclaw-router-copy";
  } else {
...
const stats = { total: 0, byTier: {}, estimatedCost: 0, baselineCost: 0, startedAt: new Date().toISOString() };
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url === "/stats") {
    const savings = stats.baselineCost > 0 ? ((1 - stats.estimatedCost / stats.baselineCost) * 100).toFixed(1) + "%" : "n/a";
    res.writeHead(200, { "Content-Type": "application/json" });
  ...
  req.on("end", () => {
    ...
    if (LOG_ROUTING) {
      const savings = opusCost > 0 ? ((opusCost - (estimatedTokens / 1_000_000) * cost.input) / opusCost * 100).toFixed(0) : 0;
      console.log(`[router] ${decision.tier.padEnd(6)} → ${decision.model} | score=${decision.score.toFixed(3)} conf=${decision.confidence.toFixed(2)} | ${decision.reasoning} | -${savings}% | ${text.slice(0, 80).replace(/\n/g, " ")}...`);
    }
